...
python $CONVERT_CHECKPOINT_PATH/model.py \
    ...
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision1.5-30b-a3b/qwen3.json \
...
python $CONVERT_CHECKPOINT_PATH/model.py \
    ...
    --save_platform=huggingface \
    --megatron_path $AIAK_MAGATRON_PATH \
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision1.5-30b-a3b/vision-model.json \
    --tensor_model_parallel_size=$TP \
...
# adapter
python $CONVERT_CHECKPOINT_PATH/custom/llava_onevision1_5_30b_a3b/adapter.py \
    --load_platform=mcore \
    --save_platform=huggingface \
    --megatron_path $AIAK_MAGATRON_PATH \
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision1.5-30b-a3b/adapter.json \
    --tensor_model_parallel_size=$TP \
    --pipeline_model_parallel_size=$PP \
    --load_ckpt_path=$LOAD \
...
# vision patch
python $CONVERT_CHECKPOINT_PATH/custom/llava_onevision1_5_30b_a3b/vision_patch.py \
    --load_platform=mcore \
    --save_platform=huggingface \
    --megatron_path $AIAK_MAGATRON_PATH \
    --tensor_model_parallel_size=$TP \
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision1.5-30b-a3b/vision-patch.json \
    --load_ckpt_path=$LOAD \
    --save_ckpt_path=$SAVE_PATCH

# merge
python $CONVERT_CHECKPOINT_PATH/custom/llava_onevision1_5_30b_a3b/merge_huggingface.py \
    --megatron_path $AIAK_MAGATRON_PATH \
